...
function fbCommentPayload(commentId: string, postId = 'page-1_post-1') {
  return JSON.stringify({
    object: 'page',
    entry: [
      {
...
async function postFb(body: string, signature?: string) {
  return createApp().fetch(
    new Request('https://igbot.example.com/api/webhooks/meta/facebook', {
      method: 'POST',
      headers: signature ? { 'X-Hub-Signature-256': signature } : {},
      body,
    }),
...
describe('POST /api/webhooks/meta/facebook', () => {
  it('stores the FB comment event and enqueues it with platform facebook', async () => {
    const body = fbCommentPayload('fb-c1');
    const res = await postFb(body, await sign(body));
    expect(res.status).toBe(200);

    const db = drizzle(sqlite, { schema });
    const events = await db.select().from(webhookEvents);
    expect(events).toHaveLength(1);
    expect(events[0].instagramCommentId).toBe('fb-c1');
    expect(events[0].instagramMediaId).toBe('page-1_post-1');
    expect(events[0].eventType).toBe('fb_comment');

    expect(enqueued).toHaveLength(1);
    expect((enqueued[0] as { platform?: string }).platform).toBe('facebook');
  });

  it('rejects an invalid signature on the FB route too', async () => {
    const body = fbCommentPayload('fb-c1');
    const res = await postFb(body, 'sha256=deadbeef');
    expect(res.status).toBe(401);
    expect(enqueued).toHaveLength(0);
  });

  it('ignores non-comment feed items (likes, edits, deletes)', async () => {
      ...
      entry: [
        {
          ...
          changes: [
            ...
            {
              field: 'feed',
              value: { item: 'comment', verb: 'remove', comment_id: 'c9', post_id: 'p1' },
            },
          ],
        },
      ],
    });
    const res = await postFb(like, await sign(like));
    expect(res.status).toBe(200);
    expect(enqueued).toHaveLength(0);
  });
});
