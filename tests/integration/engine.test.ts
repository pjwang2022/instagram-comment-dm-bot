...
describe('processCommentEvent — Facebook 粉專', () => {
  async function seedFbWebhookEvent(commentId: string, text: string) {
    const fbPayload = {
      object: 'page',
      entry: [
        {
    ...
    await db.insert(schema.webhookEvents).values({
      id,
      eventKey: 'key-' + commentId,
      eventType: 'fb_comment',
      instagramAccountId: 'page-1',
      instagramMediaId: 'fb-post-1',
      instagramCommentId: commentId,
      rawPayload: JSON.stringify(fbPayload),
      signatureValid: 1,
    });
    return id;
  }

  const fbMsg = (commentId: string, evtId: string) => ({
  ...
  async function seedFbAccountMediaAutomation() {
    ...
    await db.insert(schema.instagramMedia).values({
      id: 'fb-media',
      platform: 'facebook',
      instagramAccountId: 'fb-acct',
      instagramMediaId: 'fb-post-1',
      mediaType: 'POST',
    });
    await db.insert(schema.automations).values({
      id: 'fb-auto',
      instagramMediaId: 'fb-media',
      platform: 'facebook',
      name: 'fb 活動',
      status: 'active',
      matchType: 'contains_any',
  ...
  it('replies via the FB endpoints using the FB client', async () => {
    await seedFbAccountMediaAutomation();
    const evt = await seedFbWebhookEvent('fb-c1', '我想要連結');

    const igFetch = vi.fn() as unknown as typeof fetch;
    const fbFetch = vi.fn(async () =>
      new Response(JSON.stringify({ id: 'x', message_id: 'm' }), { status: 200 }),
    ) as unknown as typeof fetch;
    const deps = {
      db,
      metaClient: new MetaClient({ accessToken: 'ig', graphApiVersion: 'v21.0', fetchImpl: igFetch }),
      fbMetaClient: new MetaClient({
        accessToken: 'fb',
    ...
    const outcome = await processCommentEvent(deps, fbMsg('fb-c1', evt));
    expect(outcome.kind).toBe('completed');

    const calls = (fbFetch as unknown as { mock: { calls: [string, RequestInit?][] } }).mock.calls;
    expect(calls).toHaveLength(2); // 公開回覆 + 私訊
    expect(calls[0][0]).toContain('graph.facebook.com');
    expect(calls[0][0]).toContain('/fb-c1/comments'); // FB 公開回覆端點（IG 是 /replies）
    expect(calls[1][0]).toContain('/page-1/messages'); // 私訊：Page ID
    expect(igFetch).not.toHaveBeenCalled(); // IG client 完全沒被用到
  });

  it('skips FB events when FACEBOOK_PAGE_ACCESS_TOKEN is not configured', async () => {
    await seedFbAccountMediaAutomation();
    const evt = await seedFbWebhookEvent('fb-c1', '我想要連結');
    const { client } = okClient();

    const outcome = await processCommentEvent({ db, metaClient: client }, fbMsg('fb-c1', evt));
    expect(outcome).toEqual({ kind: 'skipped', reason: 'facebook_not_configured' });
  });

  it('does not apply an Instagram account_default automation to an FB post', async () => {
    // FB 帳號與貼文，但唯一的 account_default 自動化是 IG 平台的。
    await db.insert(schema.instagramAccounts).values({
      id: 'fb-acct',
      platform: 'facebook',
      instagramAccountId: 'page-1',
    ...
    await db.insert(schema.automations).values({
      ...
      publicReplyEnabled: 1,
      privateReplyEnabled: 0,
    });
    const evt = await seedFbWebhookEvent('fb-c1', '任何留言');
    ...
    const deps = {
      db,
      metaClient: okClient().client,
      fbMetaClient: new MetaClient({ accessToken: 'fb', graphApiVersion: 'v21.0', fetchImpl: fbFetch }),
    };

    const outcome = await processCommentEvent(deps, fbMsg('fb-c1', evt));
    expect(outcome).toEqual({ kind: 'skipped', reason: 'no_active_automation' });
    expect(fbFetch).not.toHaveBeenCalled();
  });
});
